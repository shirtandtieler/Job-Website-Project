...
def update_cache(jobpost_id: int = None, seeker_id: int = None):
    ...
    #         new_score = get_score(post.id, seeker.id, False)
    #         entry = MatchScores.query.filter_by(jobpost_id=post.id, seeker_id=seeker.id).first()
    #         if entry is None:  # not yet recorded
